...
def grabData(path,column_mass,column_model,ip_header=None):
    """ 
    ...
    ab_header: None or index of header row. Default is None.
    ...
    data_models = pd.read_table(fName_models,sep=" ",header=ip_header,skipinitialspace=True)
    print("DONE")
    ...
    ext_models = pd.concat([init,data_models.iloc[:,column_mass],last])
