...
def grabData(path):
    ...
    return ModelSet(needed_data,data_solar,data_species,data_models)
...
class ModelSet():
    ...
    def __init__(self,models_df,solar_df,species_df,data_df):
    ...
    def eYield(self,e):
        ...
        result1 = 0
        result2 = 0
        envelope_mass = self.data["envelope mass"]
        for i in range(2,len(self.model)-1):
            result1 += self.model[e].iloc[i] * (self.model["total mass"].iloc[i-1] - self.model["total mass"].iloc[i]) * self.species.loc[e]["mass"] 
            result2 = self.model[e].iloc[-1] *  envelope_mass.iloc[-1]*self.species.loc[e]["mass"]
